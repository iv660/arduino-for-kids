...
- `Timer` из урока 05 отсчитывает время. Мы добавим ему метод `getRemainingTime()`.
...
В `loop()` уже есть готовая структура из урока 06. Тебе нужно самостоятельно дополнить три места, где стоят комментарии `TODO`. Ориентируйся на эти комментарии и на логику работы программы.

Когда таймер работает, нужно получить оставшееся время и показать его на дисплее. Когда таймер закончился, нужно выключить реле, остановить таймер и показать выбранное время на дисплее. Когда кнопка нажата, код уже готов.

#### Шаг 2: Добавь метод `Timer.getRemainingTime()`
...
Напиши метод `getRemainingTime()` в классе `Timer`. Он должен возвращать оставшееся время в миллисекундах:
...
- **`getRemainingTime()`** — метод класса `Timer`, который возвращает оставшееся время в миллисекундах.
